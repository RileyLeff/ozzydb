...
/// temporary key, and returns a presigned GET URL (1-hour TTL).
pub async fn prepare_secrets(
    ...
    // Generate a presigned GET URL (1 hour TTL)
    let url = storage
        .presigned_get_url_by_key(&r2_key, std::time::Duration::from_secs(3600))
        .await?;
